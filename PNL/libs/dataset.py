...
class PersonDatasetMem(Dataset):
    def __init__(self, data_dir, key_path, transform=None):
        super(PersonDatasetMem, self).__init__()
        self.data_dir = data_dir
        self.key_path = key_path
        self.transform = transform
        if not os.path.exists(self.data_dir):
    ...
    def pre_load_lmdb(self):
        self.items = []
        tic = time.time()
        with self.env.begin(write=False) as txn:
            for i, key in enumerate(self.keys[:1000000]):
                if i % 100000 == 0:
                    percent = (i / self.num) * 100
                    using = time.time() - tic
                    print('[{:.2f}%][{:8d}|{:8d}] using {:.3f}s ...'.format(percent, 
                            i, self.num, using))
                buf = txn.get(key.encode('ascii'))
                img_flat = np.frombuffer(buf, dtype=np.uint8)
                self.items.append(img_flat)

    def __getitem__(self, index):
        vid = self.vids[index]
        img_flat = self.items[index]
        im = cv2.imdecode(img_flat, cv2.IMREAD_COLOR)
        im = cv2.cvtColor(im, cv2.COLOR_BGR2RGB)
        if self.transform is not None:
            im = Image.fromarray(im)
...
if __name__ == '__main__':
    # train_dir = '/home/store1/dengpanfu/Youtube/up_lmdb'
    # info_path = '/home/store1/dengpanfu/Youtube/key_vid.pkl'
    # dataset = PersonDatasetMem(train_dir, info_path)
    # for data in dataset:
    ...
    import pdb; pdb.set_trace()  # breakpoint 85101fe0 //
